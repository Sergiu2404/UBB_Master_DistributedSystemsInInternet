...
/*** From source: smart_lighting.pml ***/
...
{	int j, h = now._nr_pr;
	...
	case 5:	/* claim select */
		...
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim never_0 (2), from state 5\n");
		if (whichclaim != -1)
	...
	case 1:	/* Controller */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->_5_signal = 0;
#ifdef VAR_RANGES
		logval("Controller:signal", ((P1 *)pptr(h))->_5_signal);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
iniglobals(void)
{
		...
		now.personPresent = 0;
		waitingAck = 0;
		now.sensor_to_controller = addqueue(1, 1);
		now.controller_to_sensor = addqueue(2, 1);
#ifdef VAR_RANGES
		logval("lightIsOn", now.lightIsOn);
		logval("personPresent", now.personPresent);
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sensor_to_controller) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sensor_to_controller) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sensor_to_controller) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sensor_to_controller) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.controller_to_sensor) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.controller_to_sensor) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.controller_to_sensor) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.controller_to_sensor) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan sensor_to_controller (=%d):	len %d:\t", now.sensor_to_controller, q_len(now.sensor_to_controller));
	c_chandump(now.sensor_to_controller);
	printf("	chan controller_to_sensor (=%d):	len %d:\t", now.controller_to_sensor, q_len(now.controller_to_sensor));
	c_chandump(now.controller_to_sensor);
	printf("	bit    lightIsOn:	%d\n", now.lightIsOn);
	printf("	bit    personPresent:	%d\n", now.personPresent);
}
void
...
{
	switch (x) {
	case 1: Printf("requestStatus"); break;
	case 2: Printf("acknowledgeSensor"); break;
	case 3: Printf("roomEmpty"); break;
	case 4: Printf("personDetected"); break;
	default: Printf("%d", x);
	}
}
void
